...
export function useCatalogViewState(initialView: CatalogView = 'table'): CatalogViewState {
  ...
  const getInitialView = useCallback((): CatalogView => {
    ...
    const storedView = storageApi.forBucket('catalog-cards').get<CatalogView>('view');
  ...
  const setView = useCallback((newView: CatalogView) => {
